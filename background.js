...
async function processLearningData(data) {
  try {
    ...
    if (analysis) {
      ...
      if (existingTopic) {
        // UPDATE existing topic - add as subtopic
        await addSubtopic(existingTopic.id, data, analysis);
        console.log('🔄 Added subtopic to:', analysis.mainTopic);
      } else {
        // CREATE new topic
        const entry = {
          id: generateId(),
          ...
          subtopics: [] // NEW: Array to store related subtopics
        };
...
// NEW: Add subtopic to existing main topic
async function addSubtopic(mainTopicId, newData, analysis) {
  return new Promise((resolve) => {
    chrome.storage.local.get(['learningHistory'], (result) => {
      const history = result.learningHistory || [];
      const index = history.findIndex(item => item.id === mainTopicId);
      ...
      if (index !== -1) {
        ...
        if (!history[index].subtopics) {
          history[index].subtopics = [];
        }
        
        // Check if this subtopic already exists (by URL)
        const existingSubIndex = history[index].subtopics.findIndex(
          sub => sub.url === newData.url
        );
        
        if (existingSubIndex !== -1) {
          // Update existing subtopic
          history[index].subtopics[existingSubIndex].timeSpent += newData.timeSpent;
          history[index].subtopics[existingSubIndex].learnedAt = newData.timestamp;
        } else {
          // Add new subtopic
          history[index].subtopics.unshift(subtopic);
        }
        
        // Update main topic stats
        history[index].lastReviewed = newData.timestamp;
        history[index].reviewCount = (history[index].reviewCount || 0) + 1;
        
        chrome.storage.local.set({ learningHistory: history }, () => {
          console.log(`📚 Subtopic added: "${newData.title}" under "${history[index].mainTopic}"`);
          resolve();
        });
      } else {
        resolve();
...
chrome.alarms.create('backupReminder', { periodInMinutes: 1440 }); // Daily check

chrome.alarms.onAlarm.addListener((alarm) => {
  if (alarm.name === 'checkForgotten') {
    checkForgottenTopics();
  } else if (alarm.name === 'backupReminder') {
    checkBackupReminder();
  }
});
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === 'install') {
    ...
    console.log('🔄 KDP updated to new version');
    migrateData(); // Handle data migration if needed
  }
});

// NEW: Migrate old data structure to new format
async function migrateData() {
  chrome.storage.local.get(['learningHistory'], (result) => {
    const history = result.learningHistory || [];
    let migrated = false;
    
    // Add subtopics array to old entries if missing
    history.forEach(entry => {
      if (!entry.subtopics) {
        entry.subtopics = [];
        migrated = true;
...
// NEW: Backup reminder (every 30 days)
function checkBackupReminder() {
  chrome.storage.local.get(['learningHistory', 'lastBackupReminder'], (result) => {
    const history = result.learningHistory || [];
    const lastReminder = result.lastBackupReminder || 0;
    const now = Date.now();
    
    // Remind every 30 days if data exists
    if (history.length > 10 && now - lastReminder > 30 * 24 * 60 * 60 * 1000) {
      chrome.notifications.create({
        type: 'basic',
        iconUrl: 'icons/icon48.png',
        title: '💾 Backup Reminder',
        message: `You have ${history.length} topics tracked. Consider backing up your data!`,
        priority: 1,
        requireInteraction: true
      });
      
      chrome.storage.local.set({ lastBackupReminder: now });
    }
  });
}
